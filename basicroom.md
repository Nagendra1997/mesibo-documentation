...
- Familiarity with mesibo API. Read the [First Tutorial](https://mesibo.com/documentation/tutorials/first-app/#preparation) to understand how to create users and tokens
...
3. Stream my video, to the group.
4. View the videos of participants in the group

### Basic Conferencing Demo

Download the [Source Code](https://github.com/mesibo/conferencing/basic-demo). A recommended way is to clone the repo.

```
$ git clone https://github.com/mesibo/conferencing/basic-demo
```

### UI Elements needed for group calling
We will display the local video stream, the remote video streams, buttons for muting audio/video streams, a button to share our screen, and exit the group or hangup.
...
We need users to be a part of a group. So, let us first create some users. 
...
To create our first user, enter the address as `user0` and the app id as `basicroom`. Then click `Create`. The `user0` should now be displayed in the table. Set `MESIBO_APP_ID` as `basicroom` in `demo.js`.  
...
Now for a user to log in, the user needs a token. To get a token for a user, click on the `Edit` icon. Now in the `Edit User`
page, copy the `Auth Token`. Replace the `TOKEN_USER_0` with the token you just copied. Replace `ADDRESS_USER_0` with `user0`.
...
var demo_users = [
    {
     'token' : '6e4a59ab53b82c8edd149ac591ae661391f2'
     ,'address'  : 'user0'
     ,'name'    : 'User-0' 
    },

    {
     'token' : '2d7a9876e9a20847f18cf32dfbeaa13612f'
     ,'address'  : 'user1'
     ,'name'    : 'User-1' 
    },

    {
     'token' : 'a2a403cd731fd305cb75fe4a00a39e11135d90'
     ,'address'  : 'user2'
     ,'name'    : 'User-2' 
    },

    {
     'token' : 'c5701f652721b8388e0e148a8d7e22136130'
     ,'address'  : 'user3'
     ,'name'    : 'User-3' 
    },

    {
     'token' : 'a1f65cb75fe4a5288ebf97ab8f148a8d7e22136130'
     ,'address'  : 'user4'
     ,'name'    : 'User-4' 
    },
...
In the members table notice that there are no members/participants and the table is empty. We need to add group members to have users in the conference. The conference room is just a group with a group calling between members of the group. 
